...
def sum_to_n? arr, n
  arr.combination(2).any? { |a, b| a + b == n } # use combination to check if two number of array is equal to n(use a Bloack as condition)
end
...
def binary_multiple_of_4? s
  ...
  num % 4 == 0 # check if value is a multiple of 4
end
...
class BookInStock
  ...
  def initialize(isbn, price)
    ...
    raise ArgumentError, 'Price must be greater than zero' if price <= 0 # check if the price is lower than zero then error
